...
* **해결책:** 사용자의 목표와 제약 조건(월 저축 가능액)을 입력받아 달성 확률을 시뮬레이션하고, 목표 달성을 위한 최소 필요 적립액(Required PMT)과 최적의 조정안을 역산하여 제안하는 엔진을 개발
...
| 구분 | 기능명 | 상세 내용 | 비고 |
| --- | --- | --- | --- |
| **핵심 (Core)** | **TVM 계산 엔진** | 복리, 물가상승률, 세후 이자율을 적용하여 미래 가치(FV) 및 현재 가치(PV)를 정밀 계산 | Python `numpy` 활용 |
| **핵심 (Core)** | **Gap 분석 및 역산** | 목표 달성 실패 시, 부족한 금액(Gap)을 계산하고 이를 메우기 위한 **'필요 월 저축액'** 자동 산출 | 최적화 로직 |
| **핵심 (Core)** | **동적 시뮬레이션** | 슬라이더 조정을 통해 기간 연장, 금액 증액 시나리오를 실시간 차트로 시각화 | Streamlit 활용 |
...
4. **Solution:** 시스템이 3가지 대안 제시.
* A안 (금액 조정): "월 34만 원을 더 저축하세요."
* B안 (기간 조정): "목표 시기를 11개월 늦추세요."
* C안 (수익률 조정): "투자 수익률 9.2% 상품으로 변경하세요."


5. **Action:** 사용자가 슬라이더를 움직여 현실적인 타협안(월 110만 원 + 6개월 연장)을 도출하고 최종 플랜 확정.

5. 엣지 케이스 및 방어 로직 

* **Case 1: 비현실적 목표 (Over-Goal)**
* 상황: 월 10만 원으로 1년 뒤 10억 모으기 입력.
* 처리: `Infinity` 반환 대신, "현재 저축액으로는 833년이 걸립니다"라는 현실적인 피드백 메시지 출력 및 최대 가능 금액 가이드.
...
### 6.3 로직 구현 및 교차 검증
#### 1. TVM(Time Value of Money) 계산 로직
* 활용: 
    ...
    * 역할을 부여하여 "금융 시스템 관점에서 발생 가능한 치명적 엣지 케이스"들에 대하여 식별하도록 지시함
    ...
    | **데이터 타입** | `float` 자료형 사용 시 부동소수점 오차로 인해 금전적 정합성이 깨질 위험 | **[Decision]** 프로토타입 단계에서는 속도를 위해 `float`를 유지하되, 주석(Comment)으로 `Decimal` 도입 필요성을 명시하여 기술적 부채로 관리함 |
    | **엣지 케이스** | 기간(Years)에 음수나 실수가 입력될 경우 검증 로직 부재 | **[Fix]** 입력단(Pydantic/Form)에서 양의 정수만 받도록 강제하고, 백엔드 로직에서도 `abs()` 처리 추가 |

    * 제안 코드가 금융적 확장을 고려하지 않은 것을 발견
        1. End of Period만 고려
        2. 실질적 구매력 보존 및 세후 수익률이 제대로 고려되지 않았음
    * 금융적 확장 내용을 고려하자 validation 관련 코드가 사라져서, 이를 수동으로 보정함
