...
### 6.3 로직 구현 및 교차 검증
#### 1. 초안 TVM(Time Value of Money) 설계 및 기본 로직 검증 (AI 협업)
...
| **데이터 타입** | `float` 자료형 사용 시 부동소수점 오차로 인해 금전적 정합성이 깨질 위험 | **[Fix]**  감당할 만한 리소스 증가라 판단, `Decimal` 도입하여 수정  |
| **엣지 케이스** | 기간(Years)에 음수나 실수가 입력될 경우 검증 로직 부재 | **[Fix]** 입력단(Pydantic/Form)에서 양의 정수만 받도록 강제하고, 백엔드 로직에서도 `abs()` 처리 추가 |

#### 2. 금융 로직 정교화: 실질 구매력 및 세후 수익률 반영
